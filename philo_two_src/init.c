...
/*   Updated: 2021/06/01 14:55:11 by ldavids          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_philo(t_struct *glo)
{
	...
	if (glo->sem_philo_dead == SEM_FAILED || glo->sem_philo_display == SEM_FAILED \
	|| glo->sem_waiter == SEM_FAILED || glo->sem_start == SEM_FAILED \
	|| glo->sem_forks == SEM_FAILED)
	{
		free(glo->philo);
		return (1);
	}
